...
export const metadata: Metadata = pageMeta({
  title: "Reverse factoring software for buyer programs",
  description:
    ...
    <>
      ...
      <PageHero
        ...
        lede={
          <>
            <p>
              This is written for the funder running the program: the bank desk, the specialty finance
              ...
              join somebody else&rsquo;s program.
            </p>
            <p>
        ...
        aside={
          <Card accent="signal">
            ...
            <p className="mt-3 text-[0.9375rem] leading-[1.65] text-[var(--fg-muted)]">
              ...
              confirming program it is the design. The whole book leans on one obligor and that is what
              you sold.
            </p>
            <p className="mt-4 text-[0.9375rem] leading-[1.65] text-[var(--fg-muted)]">
              ...
              velocity, utilization against the program limit and every material event triggering a
      ...
      >
        ...
        <p>
          ...
          language the supplier works in. A program fails at onboarding far more often than it fails at
          credit.
        </p>
        <p>
          ...
          withdrawal conditions are understood, and that the set off terms in the program agreement are
      ...
      <FeatureGrid
        ...
        title="Six things a program lives or dies on."
        items={[
          {
          ...
          },
          {
            ...
            body: "The single most expensive detail in the program. The machine may stop the payment and never release it. A named person releases, under four eyes, with the reason recorded.",
          },
          {
          ...
          {
            title: "Program limit and utilization",
            body: "Utilization against the buyer's limit and against your own facility, reported continuously with availability compression and days to zero on the current trajectory.",
          },
          {
      ...
      <StepList
        eyebrow="The cycle"
        title="From an approved payable to a settled program."
        ...
        steps={[
          ...
          },
          {
            ...
            body: "The discount is computed from the tenor remaining to maturity and the program pricing, and the offer is presented in the supplier's own language and currency where the program runs across borders. What the supplier sees is what the platform recorded.",
          },
          {
            label: "Acceptance",
            title: "Acceptance is per invoice, and it is a decision the supplier owns",
            body: "Some suppliers take every offer, some take one in five at quarter end, and the difference is the economics of your program. Acceptance is linked to the offer that produced it, at the price that stood on the day, so nothing is repriced retrospectively.",
          },
          {
            label: "Payment",
          ...
          {
            label: "Maturity",
            title: "The buyer settles, and the program reconciles itself",
            body: "Settlement is matched against the approved payables it discharges, with the original remittance preserved as evidence and never posted silently. Buyer payment velocity feeds straight back into the underwriting run on the one name the whole program depends on.",
          },
        ]}
      />
          ...
          <div className="mt-11">
            <DataTable
              ...
              rows={[
                ...
                ["Concentration", "A warning to be managed and reported", "The intended design of the program"],
                ...
                ["Notification", "Sometimes withheld, and constrained everywhere when it is", "Inherent. The buyer created the program and the supplier joined it"],
      ...
      <ProblemSolution
        ...
        title="Programs rarely fail loudly. They stall."
        rows={[
          {
            problem:
              "Two hundred suppliers were onboarded and forty use the program, and nobody can say why the other hundred and sixty do not.",
            response:
              "Uptake is measured per supplier and per tenor against offers made, so a program that has been sold but not adopted is visible as a number rather than as a feeling.",
          },
          {
            problem:
          ...
          {
            problem:
              "The buyer's payment behavior changes and the first anybody hears is a settlement that arrives four days late.",
            response:
      ...
      <FaqBlock items={FAQS} title="What a program owner asks first" />
      ...
      <RelatedPages
        links={[
          ...
          { href: "/platform/risk-monitoring", label: "Risk monitoring", note: "Payment velocity, utilization and concentration on a single obligor." },
      ...
      <CtaBand
        title="Bring us a program that is not converting."
...
const PIPELINE: { stage: string; count: string; note: string; kind: "available" | "attention" | "critical" | "info" }[] = [
  { stage: "Invited", count: "312", note: "Program agreement sent in the supplier's own language", kind: "info" },
    ...
    <figure className="m-0">
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line)] bg-[var(--bg-sunken)] px-5 py-3.5">
          <div>
            <p className="font-mono text-[0.625rem] uppercase tracking-[0.13em] text-[var(--fg-subtle)]">
              Program CONF-04 &middot; buyer Astilla Comercial &middot; confirming
            </p>
        ...
        <p className="border-t border-[var(--line)] bg-[var(--bg-sunken)] px-5 py-3 font-mono text-[0.625rem] uppercase tracking-[0.11em] text-[var(--fg-subtle)]">
          Buyer limit utilization 71% &middot; approved payables unsettled 0 &middot; last underwriting run 2 days ago
        </p>
      </div>
      <figcaption className="mt-3 max-w-[52ch] text-[0.75rem] leading-[1.5] text-[var(--fg-subtle)]">
        Illustration of the supplier onboarding pipeline in a confirming program. Onboarding as states with
        a named blocking requirement, the human only hold on bank details, uptake measurement and limit
        utilization are the platform&rsquo;s own behavior. Buyer name and figures come from a seeded demonstration
        book.
      </figcaption>
    </figure>
  );
