...
// Every page advertises an og:image. The cards are generated by `npm run og`
// and committed, so a page added without running it ships a card that 404s and
// the link preview on LinkedIn or in Teams falls back to nothing. This refuses
// to finish rather than let that reach production.
const ogRefs = new Map();
(function scan(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    for (const m of html.matchAll(/og:image" content="https:\/\/factorfox\.com(\/[^"]+)"/g)) {
      if (!ogRefs.has(m[1])) ogRefs.set(m[1], "/" + path.relative(OUT, full).replace(/index\.html$/, ""));
    }
  }
})(OUT);
const missingCards = [...ogRefs].filter(([asset]) => !fs.existsSync(path.join(OUT, asset)));
if (missingCards.length) {
  console.error(`\n${missingCards.length} social card(s) referenced but not in the build:`);
  for (const [asset, page] of missingCards) console.error(`  ${asset}   referenced by ${page}`);
  console.error("Run `npm run og` and commit public/og. Do not deploy a page whose link preview is a 404.");
  process.exit(1);
}
console.log(`Social cards present: ${ogRefs.size} referenced, all found`);
